...
source_elements_xpath = "|".join(
    [f"//tei:body//tei:{el_name}" for el_name in source_elements]
)
...
def tag_doc(docpath: str, source_elements_xpath: str):
    ...
    for source_element in source_elements:
        if source_element.text is not None and source_element.text.strip() != "":
            text = source_element.text
            tokens = nl_processor(text)
            source_element.text = "\n"
            for token in tokens:
                if token.pos_ == "PUNCT":
                    subel = teiMaker.pc(
        ...
        if source_element.tail is not None and source_element.tail.strip() != "":
            text = source_element.tail
            tokens = nl_processor(text)
            source_element.tail = ""
            token_tags = []
            for token in tokens:
                ...
                token_tags.append(subel)
            token_tags.reverse()
            for token_tag in token_tags:
                source_element.addnext(token_tag)
            source_element.tail = "\n"
